 ...
 */
// we will copy data from pBitStreamBuf, not read from file
static inline int getChunk(ANNEXB_t *annex_b)
{
    ...
    if (annex_b->iBitStreamSize > annex_b->iIOBufferSize) {
        memcpy(annex_b->iobuffer, annex_b->pBitStreamBuf, annex_b->iIOBufferSize);
        annex_b->pBitStreamBuf += annex_b->iIOBufferSize;
        annex_b->iBitStreamSize -= annex_b->iIOBufferSize;
        readbytes = annex_b->iIOBufferSize;
    }
    else {
...
void open_annex_b (char *fn, ANNEXB_t *annex_b)
{
  ...
  /* read bitstream from buffer, won't open a file
...
void put_data_in_annex_b(ANNEXB_t *annex_b, byte *buf, int size)
{
    annex_b->pBitStreamBuf = buf;
    annex_b->iBitStreamSize = size;
    annex_b->is_eof = FALSE;
}
